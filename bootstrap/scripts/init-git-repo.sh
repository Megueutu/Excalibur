#!/usr/bin/env bash
# bootstrap/scripts/init-git-repo.sh — initializes a git repository at the given path.
# Usage: bootstrap/scripts/init-git-repo.sh <path>
...
if [[ -z "$TARGET_PATH" ]]; then
  echo "Usage: bootstrap/scripts/init-git-repo.sh <path>" >&2
  exit 1
fi
