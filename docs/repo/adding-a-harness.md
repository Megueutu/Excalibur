...
The CLI's build step writes the effective (override-resolved) files to the fixed path the harness reads. For Claude that's `.claude/agents/` and `.claude/skills/`. A new harness means adding its target paths to `cli/src/lib/paths.js` — that's the only place in the CLI that should know them.
