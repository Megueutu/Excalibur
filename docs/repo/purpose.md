...
- **Not installed by cloning.** The CLI installs it (`npx @spec/excalibur init`), the same way OpenSpec, Spec Kit and BMad Method all do it.
...
| | Where | Versioned? |
|---|---|---|
| **The framework's machinery** | `node_modules/@spec/excalibur/` + the overrides folder (`custom_dir`) + the config (`excalibur.yaml` or `package.json`'s `"excalibur"` key) | `node_modules/@spec/excalibur/` no, the rest yes |
