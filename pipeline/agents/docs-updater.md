---
name: docs-updater
description: Use after `wizard/scripts/scan-spec-freshness.sh` has produced a candidate list, to judge which candidates are real spec drift and which are noise. Not for finding candidates itself — that's the deterministic script's job — and not for fixing the drift; it reports, and updates only `freshness_check`.
...
The candidate YAML from `wizard/scripts/scan-spec-freshness.sh` (via `/docs-update`), and nothing else pre-loaded. For each candidate you decide is worth a real look, pull the actual diffs yourself:
