...
| [`lib/agents/`](../agents/) | The internal agent catalog — see [`lib/agents/README.md`](../agents/README.md) |
...
The orchestrator decides who does what and builds each handoff. Everything below it runs as an isolated subagent: it gets the handoff and nothing else — not the user's session, not the execution history. That isolation is the point, not a side effect (see [`lib/agents/README.md`](../agents/README.md)).
