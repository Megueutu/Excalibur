...
1. **Confirm the project is onboarded.** Is there an `excalibur.yaml` config file at the root, or an `"excalibur"` key in `package.json`? If not, check `.sdd/` and a sibling `<repo>-sdd/` (see "Detection" in [`onboarding/flow.md`](../../../../onboarding/flow.md)). If none exist, stop and offer `/excalibur-init` — don't improvise a structure.
