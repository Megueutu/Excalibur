...
1. **Deterministic sweep — free, no LLM.** Run:
   ...
   `<sdd-path>` is the project's SDD destination (where `specs/` lives). This produces a YAML candidate list from commit history, tags and `depends_on` alone — see `pipeline/agents/docs-updater.md` and `rules/writing-md-obsidian.md#anti-spec-drift-frontmatter` for what the fields mean. If it returns no candidates, say so and stop — there is nothing for judgment to do.
