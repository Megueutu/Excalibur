---
name: revert
description: Use to start work on undoing a previous change in a project onboarded with Excalibur — `/revert <what you want>`. It is the explicit entry point to the pipeline, carrying the effort expected of a `revert` task so the orchestrator doesn't have to infer it from prose. Not for a task of a different type; pick the skill matching what the work actually is.
---

# /revert

Entry point to the Excalibur pipeline for a **`revert`** task.

## Effort hint

> Very light — mechanical, almost no decision-making involved.
