...
2. **Confirm the project isn't already onboarded.** See "Detection" in [`wizard/entrypoint.md`](../../../../wizard/entrypoint.md). If it is, stop and say so — don't run onboarding on top of an existing destination.

3. **Follow [`wizard/entrypoint.md`](../../../../wizard/entrypoint.md) exactly.** Git/GitHub setup, the manifest questions (or their defaults, if the user picks that), the post-manifest interpretation step, the copy list, `init.sh`, and translation if the language isn't English.
