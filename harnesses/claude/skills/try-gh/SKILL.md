...
A friendly diagnostic on top of `wizard/scripts/check-gh.sh`.

## The order matters: cheap first

1. **Local check — free.** Run `wizard/scripts/check-gh.sh` and read its exit code:
...
2. **Local install path — still free.** Run `wizard/scripts/detect-os.sh` and use the matching installer already in `wizard/scripts/` (`install-gh-windows.sh`, `install-gh-macos.sh`, `install-gh-linux-apt.sh`, `install-gh-linux-dnf.sh`). Ask before running it, unless the project's autonomy setting says otherwise.
