#!/usr/bin/env bash
# wizard/scripts/detect-stack.sh — guesses a project's language, framework and IDE
# from files that are already on disk, so onboarding doesn't have to ask.
#
# Usage: wizard/scripts/detect-stack.sh <target-path>
...
if [[ -z "$TARGET" || ! -d "$TARGET" ]]; then
  echo "Usage: wizard/scripts/detect-stack.sh <target-path>" >&2
  exit 1
fi
