#!/usr/bin/env bash
# wizard/scripts/check-git-repo.sh — checks whether a path is itself the root of a git repository (has its own .git).
...
# Usage: wizard/scripts/check-git-repo.sh <path>
...
if [[ -z "$TARGET_PATH" ]]; then
  echo "Usage: wizard/scripts/check-git-repo.sh <path>" >&2
  exit 2
fi
