#!/usr/bin/env bash
# wizard/scripts/setup-new-repo.sh — finishes setting up a brand-new GitHub repo,
# right after wizard/scripts/create-github-repo.sh, branching on `github_preset`
# from wizard/manifest.yaml (values: conservative/direct/custom).
#
# Usage: wizard/scripts/setup-new-repo.sh <path> <repo-name> <conservative|direct|custom> [description]
...
if [[ -z "$TARGET_PATH" || -z "$REPO_NAME" ]]; then
  echo "Usage: wizard/scripts/setup-new-repo.sh <path> <repo-name> <conservative|direct|custom> [description]" >&2
  exit 1
fi
