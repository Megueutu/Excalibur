#!/usr/bin/env bash
# scripts/convert-history.sh — converts task history into a compact digest.
#
# Usage: scripts/convert-history.sh <sdd-destination> [output-file]
...
if [[ -z "$DEST" || ! -d "$DEST" ]]; then
  echo "Usage: scripts/convert-history.sh <sdd-destination> [output-file]" >&2
  exit 1
fi
...

{
  ...
  echo "# Regenerate with: scripts/convert-history.sh <sdd-destination>"
  echo "#"
