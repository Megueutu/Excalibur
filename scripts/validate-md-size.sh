#!/usr/bin/env bash
# scripts/validate-md-size.sh — mechanical check of .md files under a target
...
#
# Usage: scripts/validate-md-size.sh <target-dir>
...
# (scripts/) and materialized into a target project (.excalibur/scripts/)
...
if [[ -z "$TARGET" || ! -d "$TARGET" ]]; then
  echo "Usage: scripts/validate-md-size.sh <target-dir>" >&2
  exit 2
fi
...
# project's own minimal-YAML philosophy (see packages/cli/core/yaml.js).
RECORDS="$(awk '
  { gsub(/\r$/, "") }
