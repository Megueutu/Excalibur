#!/usr/bin/env bash
# scripts/scan-spec-freshness.sh — deterministic sweep for specs that may have drifted
...
#
# Usage:
#   scripts/scan-spec-freshness.sh <sdd-path> [--repo <git-repo-path>] \
...
if [[ -z "$SDD_PATH" || ! -d "$SDD_PATH" ]]; then
  echo "Usage: scripts/scan-spec-freshness.sh <sdd-path> [--repo <path>] [--age-threshold-days N] [--output <file>]" >&2
  exit 1
fi
