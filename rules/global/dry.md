...
- `lib/onboarding/`, `lib/pipeline/` and `rules/` are the single source of truth for content. `packages/cli/` reads them at run time from wherever npm installed the package and keeps no second copy.
