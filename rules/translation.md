...
| **Operational Excalibur** | **never** | the installed package (`node_modules/@easy-spec/excalibur/`) entirely (rules, agent prompts, session directives), `lib/pipeline/`, `rules/`, `tasks.yaml`, `history.yaml` |
...
English. Everything under `lib/pipeline/`, `rules/` and the installed package (`node_modules/@easy-spec/excalibur/`) is written and kept in English regardless of the project's language, so the same framework content works for every project and never needs a translation pass.
