#!/usr/bin/env bash
# lib/scripts/check-git-repo.sh — checks whether a path is itself the root of a git repository (has its own .git).
...
if [[ -z "$TARGET_PATH" ]]; then
  echo "Usage: lib/scripts/check-git-repo.sh <path>" >&2
  exit 2
fi
