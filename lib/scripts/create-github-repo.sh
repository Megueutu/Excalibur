#!/usr/bin/env bash
# lib/scripts/create-github-repo.sh — creates a GitHub repository for an existing local repo.
...
if [[ -z "$TARGET_PATH" || -z "$REPO_NAME" || ( "$VISIBILITY" != "public" && "$VISIBILITY" != "private" ) ]]; then
  echo "Usage: lib/scripts/create-github-repo.sh <path> <name> <public|private>" >&2
  exit 1
fi
