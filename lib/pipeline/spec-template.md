...
Never written by hand — `scripts/record-history.sh` owns the format. A changelog per task, not a granular event log, and **not read by any agent** unless a handoff or the user explicitly asks for it.
