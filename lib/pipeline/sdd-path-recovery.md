...
2. **If the path is missing or no longer exists, scan for the marker instead of
   ...
   materialized (see `cli/src/commands/init.js` / `cli/src/lib/config.js` for
   `embedded`/`separate`, and `wizard/init.sh` for whichever step materializes
   `external`/`new_repo`). List first-level folders at the project root and check each
