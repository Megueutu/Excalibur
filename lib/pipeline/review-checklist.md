...
2. On **Feature** / **Big feature**: dispatch the `review` agent (`lib/agents/review/agent.yaml`) with the diff and this file, and ask for a verdict item by item. A fresh agent without the "I already think I'm done" bias catches more. On a **Fix**, do it inline, but still explicitly.
...
- `history.yaml` has an entry for this task, written through `scripts/record-history.sh`, unless the session sets `no-history`.
